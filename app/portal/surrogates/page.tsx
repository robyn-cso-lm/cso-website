...
import { getSurrogates } from '@/lib/portal/profiles';
...
export default async function SurrogatesPage({ searchParams }: PageProps) {
  const page = Number(searchParams.page ?? 1);
  const { profiles, total, totalPages } = await getSurrogates({
    q:        searchParams.q,
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="font-serif text-2xl text-text-primary">Surrogates</h1>
      ...
      <ProfileTable
        rows={profiles}
        basePath="/portal/surrogates"
        emptyMessage="No surrogate profiles match your filters."
      />
