...
import { getSurrogateById } from '@/lib/portal/profiles';
...
export default async function SurrogateDetailPage({ params }: PageProps) {
  const profile = await getSurrogateById(params.id);
  if (!profile) notFound();

  return (
    <div>
      <Link href="/portal/surrogates" className="text-xs text-lavender hover:text-mid-purple">
        ← Back to Surrogates
      </Link>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="rounded-xl border border-lavender-light bg-white p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Carried Before" value={profile.hasCarriedBefore} />
            <Field label="# of Children" value={profile.numberOfChildren} />
            <Field label="BMI"           value={profile.bmi} />
            <Field label="Smoking"       value={profile.smokingStatus} />
          </div>
        </div>
